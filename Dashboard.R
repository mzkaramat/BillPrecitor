...
server <- function(input, output) {
  ...
  observeEvent(
    input$party_type_net,
  {
    filter_pred <- if (input$party_type_net == "Republican")
      'R'
    else
      'D'
    
    print(paste("Slider moved ",filter_pred))
    sql_command = paste("SELECT   type,   next_type,   avg(next_row - action_date) no_days FROM (SELECT         bill_id,         action_date,         type,         lead(action_date, 1)         OVER (           PARTITION BY bill_id           ORDER BY action_date ) next_row,         lead(type, 1)         OVER (           PARTITION BY bill_id           ORDER BY action_date ) next_type       FROM actions       ORDER BY bill_id, action_date) tb1 INNER JOIN bills ON bills.id = tb1.bill_id   inner join sponsors on sponsors.bill_id = bills.id   inner join members on members.bioguideid = sponsors.sponsor_id WHERE next_type IS NOT NULL and members.party = '",filter_pred,"' GROUP BY type, next_type;")
    #sql_command <-
